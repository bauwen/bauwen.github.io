...
var nolify = function (canvas) {
    ...
    api.resizeAspectRatioFill = function () {
        var w = Math.floor(window.innerWidth);
        var h = Math.floor(window.innerHeight);
        var sw = minWidth//Math.min(minWidth, w);// canvas.width;
        var sh = minHeight//Math.min(minHeight, h);//canvas.height;

        if (w / h > sw / sh) {
            //sw *= h / sh;
            //sh = h;
            canvas.width = minHeight * w / h;
            canvas.height = minHeight;
        } else {
            //sh *= w / sw;
            //sw = w;
            canvas.height = minWidth * h / w;
        ...
        canvas.style.width = Math.floor(w) + "px";
        canvas.style.height = Math.floor(h) + "px";
        canvas.style.left = "0px";//Math.floor((w - sw) / 2) + "px";
        canvas.style.top = "0px";//Math.floor((h - sh) / 2) + "px";
    };
