...
game.addObject("obj_levelcontroller", {
    ...
    update: function () {
        ...
        for (var i = 0; i < levelgrid.blocks.length; i++) {
            ...
            if (false && OUTLINE) {
                ctx.fillStyle = "black";
                ctx.fillRect(OFFSETX + block.realX + s - 4, OFFSETY + block.realY + s - 4, CELL - 2 * s + 8, CELL - 2 * s + 8);
            }
...
function updateGameMenu() {
    ...
    if (showIcons) {
        ctx.globalAlpha = 0.2;
        ctx.fillStyle = "black";
        ctx.fillText((lvl < 10 ? "0" : "") + lvl, ctx.canvas.width / 2 + 2, ctx.canvas.height - 45 + 2);
        ctx.globalAlpha = 1;
        
        ctx.fillStyle = "white";
        ctx.fillText((lvl < 10 ? "0" : "") + lvl, ctx.canvas.width / 2, ctx.canvas.height - 45);
    }
...
function drawLevelButton(n, x, y, selected, state) {
    ...
    ctx.globalAlpha = Math.max(0, alpha - 0.8);
    ctx.fillText(ns, x + CELL / 2 + 2, y + CELL / 2 + 2);
    ctx.globalAlpha = alpha;
    
    ctx.fillStyle = "rgb(255, 255, 255)";
    ctx.fillText(ns, x + CELL / 2, y + CELL / 2);
}
...
function loadScreen() {   
    ...
    game.loadAssets({
        ...
        sounds: {
            "snd_click": "src/audio/snd_click.wav",
            "snd_coin": "src/audio/snd_coin.wav",
            "snd_teleport": "src/audio/snd_teleport.wav",
            "snd_touch": "src/audio/snd_touch.wav",
            "snd_finish": "src/audio/snd_finish.wav",
            "snd_rotator": "src/audio/snd_rotator.wav",
            "snd_error": "src/audio/snd_error.wav"
        },
